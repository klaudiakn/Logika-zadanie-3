...
bool cmp(char *a, char *b) //returns true if two strings are equal 
{
...
char solve_not(char a, char *file_source) //solves not, based on the data included in the file file_sources 
{
	char r;
	fstream file;
	file.open(file_source, ios::in); //open the file 
...
char solve_and(char a, char b, char *file_source) //solves 'and' sentence, based in the data included in the file file_solution
{
	char r; //this will be the solution 
	fstream file;
	file.open(file_source, ios::in); //opening the file 
	if(file.good())
	{
		while(1)
		{
...
char solve_impl(char a, char b, char *file_source) //solves implication, based on the data included in the file file_source 
{
	char r;
	fstream file;
	file.open(file_source, ios::in); //opening the file 
...
char solve_or(char a, char b, char *file_source) //solves or sentence, based on the data included in the file file_source, just like solve_impl and solve_and
{
	char r;
	fstream file;
	file.open(file_source, ios::in);
	if(file.good())
	{
		while(1)
		{
...
char get_task(char **sources) //creates random task to solve (solution in the variable 'solution')
{
	srand(time(NULL));
	char a[n];
	char solution; 
...
void game(char **sources) { //the main function
	system("cls");
